...
func TestWaitingWatcher(t *testing.T) {
	...
	w.doWatch(ctx) // first sight
	w.doWatch(ctx) // stable tick 1
	w.doWatch(ctx) // stable tick 2 -> report
	if posts.Load() != 2 {
...
func TestWaitingWatcherCallbackRetry(t *testing.T) {
	...
	fail.Store(true)
	w.doWatch(ctx) // first sight
	w.doWatch(ctx) // stable -> report attempt fails
	w.doWatch(ctx) // retried, still failing
	if successes.Load() != 0 {
